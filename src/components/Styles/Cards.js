...
export const CarouselImgs = () => {
  ...
  let renderPics = (img) =>{
    ...
    switch(img){
      case 'firstImg':
        imageCode = picOne; break;
      case 'secondImg':
        imageCode = picTwo; break;
      case 'thirdImg':
        imageCode = picThree; break;
      case 'fourthImg':
        imageCode = picFour; break;
    }
    return (
      <Image src={imageCode} alt = {img} width={200} height={385} />
    )
  }
  const contentStyle = {
    height: '360px',
  ...
  return (
    <div >
        <Carousel autoplay>
            <div>
              <h3 style={contentStyle}>{renderPics("firstImg")}</h3> {/*Will need to add images from the API (These images should be adjustable by Owner)*/}
            </div>
            <div>
